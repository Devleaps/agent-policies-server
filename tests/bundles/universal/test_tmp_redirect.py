...
def test_tmp_redirect_simple(bash_event):
    ...
    assert_deny(evaluate_bash_rules, bash_event("cat data.txt > /tmp/output.log"))
    assert_deny(evaluate_bash_rules, bash_event("echo hello >> /tmp/append.txt"))


def test_tmp_redirect_safe_paths(bash_event):
    """Test that workspace redirects are allowed."""
    assert_allow(evaluate_bash_rules, bash_event("echo test > output.txt"))
    assert_allow(evaluate_bash_rules, bash_event("cat data > ./logs/output.log"))
    assert_allow(evaluate_bash_rules, bash_event("echo hello >> results.txt"))


def test_tmp_redirect_heredoc_unparseable(bash_event):
    """Quoted heredoc delimiters are denied."""
    assert_deny(evaluate_bash_rules, bash_event("cat > /tmp/test.py << 'EOF'"))
    assert_deny(evaluate_bash_rules, bash_event("cat > /tmp/script.sh << \"EOF\""))


def test_tmp_redirect_no_redirect(bash_event):
    """Test commands without redirects are allowed."""
    assert_allow(evaluate_bash_rules, bash_event("echo hello"))
    assert_allow(evaluate_bash_rules, bash_event("cat file.txt"))
